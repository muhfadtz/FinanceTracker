...
const expenseCategoryKeys = ['food', 'transportation', 'shopping', 'bills', 'entertainment', 'health', 'education', 'otherExpense'] as const;
const incomeCategoryKeys = ['salary', 'bonus', 'gift', 'investment', 'otherIncome'] as const;

type CategoryKey = typeof expenseCategoryKeys[number] | typeof incomeCategoryKeys[number];
...
const AddTransactionModal = ({ isOpen, onClose }: AddTransactionModalProps) => {
  ...
  const [category, setCategory] = useState(() => t(expenseCategoryKeys[0] as CategoryKey));
  ...
  useEffect(() => {
    if (type === 'expense') {
        setCategory(t(expenseCategoryKeys[0] as CategoryKey));
    } else {
        setCategory(t(incomeCategoryKeys[0] as CategoryKey));
    }
  }, [type, t]);
  ...
  const currentCategoryKeys = type === 'expense' ? expenseCategoryKeys : incomeCategoryKeys;
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-evvo-green-dark bg-white dark:bg-gray-700 text-gray-900 dark:text-white" required
            >
             {currentCategoryKeys.map(catKey => (
                <option key={catKey} value={t(catKey as CategoryKey)}>
                  {t(catKey as CategoryKey)}
                </option>
              ))}
            </select>
          </div>
